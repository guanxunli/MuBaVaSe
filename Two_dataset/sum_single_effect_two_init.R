...
sum_single_effect_two_init <- function(X_1, Y_1, X_2, Y_2, sigma02_int = NULL, sigma2_int = NULL, 
                                  r = 0.2, q = 0.05, tau = 1.5, L = NULL, itermax = 100, 
                                  tol = 1e-4, sigma0_low_bd = 1e-8, residual_variance_lowerbound = NULL,
                                  b_mat_1 = NULL, b_mat_2 = NULL) {
  ## Initialization
  p <- ncol(X_1)
  n <- nrow(X_1)
  ...
  # Save matrix
  if (is.null(b_mat_1)) b_mat_1 <- matrix(0, nrow = p, ncol = L)
  b2_mat_1 <- matrix(0, nrow = p, ncol = L)
  if (is.null(b_mat_2)) b_mat_2 <- matrix(0, nrow = p, ncol = L)
...
# time1 <- Sys.time()
# res <- sum_single_effect_two_init(X_1, Y_1, X_2, Y_2, sigma02_int = NULL, sigma2_int = NULL,
...
# ## with initialization
# time1 <- Sys.time()
# out_res <- sum_single_effect_two_init(X_1, Y_1, X_2, Y_2, sigma02_int = NULL, sigma2_int = res$sigma2,
#                                       r = 0.2, q = 0.05, tau = 1.5, L = p_c + p_1 + p_2, itermax = 100,
#                                       tol = 1e-4, sigma0_low_bd = 1e-8, 
#                                       b_mat_1 = res$b_mat_1, b_mat_2 = res$b_mat_2)
# Sys.time() - time1
# out_res$ELBO
