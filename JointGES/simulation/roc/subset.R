...
subset <- function(y, x, data){
	...
	#remove edges
	if(length(x) == 1){
		model <- glmnet(cbind(matrix(0, nrow(data), 1), as.matrix(data[,x])), data[,y], family="gaussian", intercept=FALSE, lambda=lambda2)
		if(sum(as.vector(coef(model)) != 0) > 0)
			t[x] <- 1
	}else{
...
save(gesdag.list, dag.list, file=paste("joint/", strsplit(basename(args[4]), ".rda", fixed=TRUE)[[1]][1], "_lambda_", toString(lambda2), ".rda", sep=""))
