...
joint_graph_multi <- function(dta_list, sigma02_int = NULL, sigma2_int = NULL, prior_vec = NULL, 
  ...
  for (iter_K in seq_len(K)) {
    ...
    Xb_list[[iter_K]] <- matrix(0, nrow = p, ncol = n)
    Y_list[[iter_K]] <- dta_list[[iter_K]][1, ]
    mean_list[[iter_K]] <- mean(dta_list[[iter_K]][1, ])
  ...
  for (iter_p in seq_len(p - 1)) {
    ...
    for (iter_K in seq_len(K)) {
      ...
      dta_vs_list[[iter_K]]$X <- t(dta_list[[iter_K]][seq_len(iter_p), , drop = FALSE])
...
# res_joint <- joint_graph_multi(dta_list, prior_vec = NULL, tau = 1.5)