...
sum_single_effect_two_single <- function(X_1, Y_1, X_2, Y_2, scale_x = TRUE, intercept = TRUE,
  ...
  if (length(index_select) > 0) {
    ...
    if (length(index1_select) > 0) {
      Sigma_1_inverse <- diag(1, n1) - X_1[, index1_select, drop = FALSE] %*%
        solve(crossprod(X_1[, index1_select, drop = FALSE]) + sigma2 * diag(1 / sigma02_select1, nrow = length(sigma02_select1)), t(X_1[, index1_select, drop = FALSE]))
    ...
    if (length(index2_select) > 0) {
      Sigma_2_inverse <- diag(1, n2) - X_2[, index2_select, drop = FALSE] %*%
        solve(crossprod(X_2[, index2_select, drop = FALSE]) + sigma2 * diag(1 / sigma02_select2, nrow = length(sigma02_select2)), t(X_2[, index2_select, drop = FALSE]))
...
# ## joint method 1
# time1 <- Sys.time()
# res_new <- sum_single_effect_two_single(X_1, Y_1, X_2, Y_2, L = p_c + p_1 + p_2 + 1, scale_x = TRUE, intercept = TRUE)
# Sys.time() - time1
#
# ## joint method 2
