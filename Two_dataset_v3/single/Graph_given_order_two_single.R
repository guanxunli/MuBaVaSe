...
joint_graph_fun_two_single <- function(dta_1, dta_2, scale_x = FALSE, intercept = TRUE,
  ...
  for (iter_p in seq_len(p - 1)) {
    ...
    Y_2 <- dta_2[, iter_p + 1]
    ## variable selection
    res <- sum_single_effect_two_single(
...
# time1 <- Sys.time()
# res_joint_new <- joint_graph_fun_two_single(dta_1 = dta_1, dta_2 = dta_2, scale_x = FALSE, intercept = TRUE)
# Sys.time() - time1
...
# #### our method 2
# adj_1_new <- t(res_joint_new$alpha_res_1)
...
# #### our method 2
# adj_2_new <- t(res_joint_new$alpha_res_2)
