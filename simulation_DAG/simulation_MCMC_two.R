...
# source("two_data_sets/Graph_MCMC_two_sim.R")
...
for (iter_prior in seq_len(length(prior_vec_list))) {
  ...
  for (iter_graph in seq_len(n_graph)) {
    ...
    if (iter_graph %% 25 == 0) {
      ...
      png(paste0(
        ...
        "e_com", e_com, "e_pri", e_pri, "data1_mcmctwo.png"
      ))
      grid.arrange(gl, gs_1, gu_1, layout_matrix = layout_matrix)
      dev.off()
      png(paste0(
        "prior", iter_prior, "graph", iter_graph,
        "e_com", e_com, "e_pri", e_pri, "data2_mcmctwo.png"
      ))
      grid.arrange(gl, gs_2, gu_2, layout_matrix = layout_matrix)
      dev.off()
    }
  ...
  # show results
  res <- (colMeans(res_1) + colMeans(res_2)) / 2
  cat(
