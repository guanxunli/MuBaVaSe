...
#   out_res <- joint_graph_fun_two_sampling(dta_1 = dta_1, dta_2 = dta_2, prior_vec = prior_vec,
#                                         scale_x = scale_x, intercept = intercept)
#   print(round(sum(out_res$llike_vec_1 + out_res$llike_vec_2) + sum(out_res$lprior_graph), 4))
#   ## Calculate the error
#   ## data set 1
#   adj_1 <- out_res$graph_res_1
#   adj_1 <- t(adj_1)
...
#     "L2", round(check_adj_l2(adj_pre = out_res$graph_res_1, adj_act = adj_true2), 4),
#     "L1", round(check_adj_l1(adj_pre = out_res$graph_res_1, adj_act = adj_true2), 4),
#     "\n"
#   )
#   ## data set 2
#   adj_2 <- out_res$graph_res_2
#   adj_2 <- t(adj_2)
...
#     "L2", round(check_adj_l2(adj_pre = out_res$graph_res_2, adj_act = adj_true2), 4),
#     "L1", round(check_adj_l1(adj_pre = out_res$graph_res_2, adj_act = adj_true2), 4),
#     "\n"
#   )
# }
# 
...
for (iter_prior in seq_len(length(prior_vec_list))) {
  ...
  out_res <- foreach(iter = seq_len(n_graph)) %dorng% {
    ...
    order_int <- as.numeric(igraph::topo_sort(graph_i))
    # Do MCMC
    Graph_MCMC_two_sim_sampling(dta_1, dta_2,
      scale_x = scale_x, intercept = intercept,
      order_int = order_int, iter_max = iter_max, sigma02_int = NULL, sigma2_int = NULL,
      prior_vec = prior_vec, itermax = 100, tol = 1e-4,
  ...
  for (iter_graph in seq_len(n_graph)) {
    ...
    if (iter_graph %% 10 == 0) {
      ...
      png(paste0(
        ...
        "e_com", e_com, "e_pri", e_pri, "data1_sampling.png"
      ))
      grid.arrange(gl, gs_1, gu_1, layout_matrix = layout_matrix)
      dev.off()
      png(paste0(
        "pri", prior_vec[1], "com", prior_vec[2], "graph", iter_graph,
        "e_com", e_com, "e_pri", e_pri, "data2_sampling.png"
      ))
      grid.arrange(gl, gs_2, gu_2, layout_matrix = layout_matrix)
      dev.off()
    }
