...
#   cat("p:", p, "e_com:", e_com, "e_pri", e_pri, "prior:", prior_vec, 
#       "data1:", c(shd(g_true1, g_1), check_edge(adj_true1, adj_1)), "\n")
#   ## data set 2
#   adj_2 <- ifelse(out_res$alpha_res_2 > 0.5, 1, 0)
#   adj_2 <- t(adj_2)
#   g_2 <- as(getGraph(adj_2), "graphNEL")
#   cat("p:", p, "e_com:", e_com, "e_pri", e_pri, "prior:", prior_vec, 
#       "data2:", c(shd(g_true2, g_2), check_edge(adj_true2, adj_2)), "\n")
# }
# 
# ########################## Do MCMC quick test ############################
# iter_max <- 100
...
for (iter_prior in seq_len(length(prior_vec_list))) {
  ...
  out_res <- foreach(iter = seq_len(n_graph)) %dorng% {
    ...
    order_int <- as.numeric(igraph::topo_sort(graph_i))
    # Do MCMC
    Graph_MCMC_two_sim(dta_1, dta_2, scale_x = scale_x, intercept = intercept,
                       order_int = NULL, iter_max = iter_max, sigma02_int = NULL, sigma2_int = NULL,
                       prior_vec = prior_vec, itermax = 100, tol = 1e-4, sigma0_low_bd = 1e-8,
  ...
  for (iter_graph in seq_len(n_graph)) {
    ...
    gl <- ggplot() + geom_line(aes(x = seq_len(iter_max), y = res_tmp$llike_vec)) +
      xlab("Iteration") + ylab("Log likelihood")
    gs_1 <- ggplot() + geom_line(aes(x = seq_len(iter_max), y = res_tmp$error_mat1[1, ])) +
      xlab("Iteration") + ylab("SHD")
    gu_1 <- ggplot() + geom_line(aes(x = seq_len(iter_max), y = res_tmp$error_mat1[2, ])) +
      xlab("Iteration") + ylab("No order")
    gs_2 <- ggplot() + geom_line(aes(x = seq_len(iter_max), y = res_tmp$error_mat2[1, ])) +
      xlab("Iteration") + ylab("SHD")
    gu_2 <- ggplot() + geom_line(aes(x = seq_len(iter_max), y = res_tmp$error_mat2[2, ])) +
      xlab("Iteration") + ylab("No order")
    png(paste0("pri", prior_vec[1], "com", prior_vec[1], "graph", iter_graph, "data1.png"))
    grid.arrange(gl, gs_1, gu_1, layout_matrix = layout_matrix)
    dev.off()
    png(paste0("pri", prior_vec[1], "com", prior_vec[1], "graph", iter_graph, "data2.png"))
    grid.arrange(gl, gs_2, gu_2, layout_matrix = layout_matrix)
    dev.off()
    # analysis
    alpha_mat_1 <- matrix(0, nrow = p, ncol = p)
...
# intercept TRUE prior: 5e-04 1e-04 data1: 31.7 20.4 data2: 29.5 18.2
# intercept TRUE prior: 0.001 0.001 data1: 36.8 22.55 data2: 34.85 21
# intercept TRUE prior: 0.001 1e-04 data1: 36.25 24.2 data2: 33.6 21.2